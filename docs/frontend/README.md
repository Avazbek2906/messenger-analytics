...
> **[CHANGELOG](./CHANGELOG.md) — read it after this page.** Docs 01–07 were written at
> one point in time; the changelog carries everything added since, and is authoritative
...
|---|---|---|
| 00 | **[Integration guide](./00-integration-guide.md)** | **Start here.** The manual, not the reference: the auth loop with working code, the five rules that cost a day each, error handling, which endpoint feeds which chart, and a pre-launch checklist. |
| 01 | **[Conventions](./01-conventions.md)** | Auth, tenancy, roles, pagination, filtering, date formats, the error envelope, CORS. The rules from doc 00 in reference form. |
