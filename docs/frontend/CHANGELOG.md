...
is what `/api/v1/dashboard/me` and the browser extension require** — a login without it
...
### 5. `/dashboard/insights` and `/dashboard/ask` now fail with a code, not a 500

These two are the only endpoints that call Gemini **inside** your request. They caught
just the missing-API-key case, so every other upstream failure — a quota `429`, an
upstream 5xx, a timeout, or model output the schema rejected — escaped as an unhandled
**500** with a traceback and no `code` to match on. A frontend could not tell "come back
in a minute" from "this is broken."

New: **`503 ai_unavailable`**.

| Status | `code` | Meaning | Handling |
|---|---|---|---|
| 503 | `ai_unavailable` | Upstream failure; **your request was valid** | Transient — offer retry, resend verbatim |
| 400 | `gemini_not_configured` | No API key on the server | Permanent — hide the AI panels |

`ai_unavailable` is common in practice, not theoretical: firing all seven insight
widgets at once on a tight quota reliably trips it. Load the panels with backoff and
give each its own error state rather than blanking the page — the rest of the dashboard
does not depend on Gemini and stays fine.

Full guidance, including `has_data` on `/ask` and the untrusted-output warning, is in
[the integration guide §8](./00-integration-guide.md#the-two-ai-endpoints--treat-them-differently-from-everything-else).
